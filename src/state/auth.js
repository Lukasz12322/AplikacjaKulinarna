...
const getUrlWithToken = () => {
    ...
    .catch(error => {
        ...
        if (refreshToken && error.response.statusText === 'Unautorized')
            return dispatch(useRefreshTokenCreator(refreshToken))
            .then(() => {
                return axios({
                    url: getUrlWithToken(),
                    method,
...
export const registerCreator = (email, password) => (dispatch, getState) => {
...
export const resetPasswordCreator = (email, success) => (dispatch, getState) => {
    dispatch(circuralProgress.add())
    ...
    .then(() => {
        dispatch(addSnackbar('Email z nowym hasłem został wysłany'))
        success()
    })
    .catch(error => {
...
export const autoLogInCreator = () => (dispatch, getState) => {
    const refreshToken = localStorage.getItem('refreshToken')

    if (refreshToken) {
        dispatch(useRefreshTokenCreator(refreshToken))
    }
}

const useRefreshTokenCreator = refreshToken => (dispatch, getState) => {
    dispatch(circuralProgress.add())
